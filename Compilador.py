...
def get_etiqueta(t):
    ...
    if t.startswith('"') and t.endswith('"'):
        ...
        return 'integer'  # Cambiado de 'entero' a 'integer'
    elif t.count('.') == 1 and all(part.isdigit() for part in t.split('.')):
        return 'real'  # Cambiado de 'flotante' a 'real'
    else:
        return 'desconocido'
...
def declaracion_variable():
    ...
    emit(f'DECLARE {var_name} : {tipo}')  # Cambiado de DECLARAR a DECLARE
...
def asignacion():
    ...
    # Manejo especial para funciones intrínsecas
...
def for_loop():
    ...
    # En for, capturamos expresiones simples (identificador o número) sin análisis posfijo
    inicio = actual()
    if inicio is None:
...
def escritura():
    ...
    for item in textos_a_imprimir:
        emit(f'PRINT {item}')  # Cambiado de 'IMPRIMIR' a 'PRINT'
...
def lectura():
    ...
    for var in variables_a_leer:
        emit(f'READ {var}')  # Cambiado de 'LEER' a 'READ'
...
# Expresión corregida que maneja paréntesis anidados correctamente
def expresion():
    infija = []
    paren_count = 0
    ...
    while True:
        ...
        if token == '(':
            ...
            if paren_count == 0:
                # Paréntesis de cierre que termina la expresión
                break
            paren_count -= 1
            infija.append(token)
            coincidir(token)
        elif token in (';', ',', '}') and paren_count == 0:
            # Delimitadores externos que terminan la expresión si no estamos dentro de paréntesis
            break
        else:
            infija.append(token)
            coincidir(token)
...
def funcion_intrinseca():
    ...
    # Capturar argumento simple sin llamar a expresion()
    arg = actual()
    if arg is None:
